...
d = 0.05	#	duration of the tone (seconds , when the sampling rate is defined by you, the device will understand what 3 seconds means !)	
...
#	sound may be either noise or tone.
##sound = np.random.uniform( low=-1 , high=1 , size=int(d*sr) )	


#	tone
dots = np.arange(0,d,1/sr)	
sound = np.sin(2*pi*f*dots)


#	noise , I   (old)  =  sound  (current)
#	noise.shape  ,  I.shape   =   (1600,)
...
#	ascending ramp. 	
#	1st 7ms.
rpr[0: int(0.007 * sr) ] = np.linspace(0,1,int(0.007 * sr))

#	descending ramp.
#	last 7ms.
rpr[ -int(0.007 * sr ) : ] = np.linspace(1,0,int(0.007 * sr))
...
zeros_1 = np.zeros(int(itl_1*sr))
zeros_2 = np.zeros(int(itl_2*sr))
zeros_3 = np.zeros(int(itl_3*sr))
zeros_4 = np.zeros(int(itl_4*sr))
zeros_5 = np.zeros(int(itl_5*sr))
zeros_6 = np.zeros(int(itl_6*sr))
zeros_7 = np.zeros(int(itl_7*sr))
zeros_8 = np.zeros(int(itl_8*sr))


#####################

#	ss = sound + silence. vectoral concatenation.
ss_1 = np.concatenate((r_s,silence_1) , axis=None)
ss_2 = np.concatenate((r_s,silence_2) , axis=None)
ss_3 = np.concatenate((r_s,silence_3) , axis=None)
ss_4 = np.concatenate((r_s,silence_4) , axis=None)
ss_5 = np.concatenate((r_s,silence_5) , axis=None)
ss_6 = np.concatenate((r_s,silence_6) , axis=None)
ss_7 = np.concatenate((r_s,silence_7) , axis=None)
ss_8 = np.concatenate((r_s,silence_8) , axis=None)
...
White socket (from the USB audio interface) is sound.


In total the experiment lasts for 30 minutes.
...
for test reasons you can use this :
    sd.play(st_4.T , sr )

####################

the trigger length must put to 4000 (with a sampling rate of 48000 instead of 16000), for the triggers to be shown in oscilloscope !!
...
plot :
plt.plot(insl_total_T[192000: 336000 , :])
